...
function parseEntries(xml: string, ccaa: string) {
  ...
  while ((match = entryRegex.exec(xml)) !== null) {
    ...
    const tipo_contrato = TIPO_MAP[tipoRaw.toLowerCase()] ?? tipoRaw.toLowerCase()
...
export async function GET(req: NextRequest) {
  ...
  const tipo       = sp.get('tipo')?.toLowerCase()
  ...
  try {
    ...
    const results = entries.filter(l => {
      ...
      if (tipo && l.tipo_contrato && !l.tipo_contrato.includes(tipo.slice(0, 5))) return false
      return true
    })
